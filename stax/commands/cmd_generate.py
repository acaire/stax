...
def generate():
    ...
    Generate a sample stax.json file
    """
    if os.path.exists('stax.json'):
        if not click.confirm("stax.json already exists, overwrite?"):
            click.echo('Skipped overwriting stax.json')
            return

    with click.open_file('stax.json', 'w') as fh:
        content = dict(
            accounts={
                "development": {
                    "id": "123",
        ...
        json.dump(content, fh, sort_keys=True, indent=4)
        click.echo('Wrote stax.json')
